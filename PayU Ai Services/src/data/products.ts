...
export function createDefaultProductConfigs(): Record<ProductId, import("../types").ProductConfig> {
  return {
    "checkout-emi": {
      enabled: true,
      minAmount: 3000,
...
export function createDefaultSettings(name: string, mid: string): import("../types").MerchantSettings {
  return {
    merchantName: name,
    mid,
    contactEmail: `${name.toLowerCase().replace(/\s+/g, "")}@merchant.com`,
    ...
    webhookUrl: "https://merchant.com/webhooks/payu",
    logoUrl: "",
    themeColor: "#10847E",
    testMode: true,
    autoCapture: true,
    ...
    products: createDefaultProductConfigs(),
    notes: "",
  };
}
